...
#
#   Copyright (C) 2002-2006, 2008, 2010, 2016, 2018 Rocky Bernstein
#   <rocky@gnu.org>
#
...
_Dbg_do_handle() {
    ...
    if [[ $sig == $int_pat ]]; then
	signame=2 #$(_Dbg_signum2name $sig) FIXME
	if (( $? != 0 )) ; then
	...
	typeset signum;
	signum=2 #$(_Dbg_name2signum $sig) FIXME
	if (( $? != 0 )) ; then
