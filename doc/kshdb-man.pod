...
B<kshdb> [I<options>] [--] I<script-name> [I<script options>]

B<kshdb> [I<options>] -c I<execution-string>

=head1 DESCRIPTION

C<kshdb> is a ksh script to which arranges for another ksh script
to be debugged. 
...
If you used kshdb script and need to pass options to the script to be
debugged, add C<--> before the script name. That will tell kshdb not
to try to process any further options.

=for comment
See the reference manual L<http://kshdb.sourceforge.net/kshdb.html>
...
C<.kshdbinit> is analogus to Perl's C<.perldb> or GNU gdb's
...
input.  These commands are read however after any C<.kshdbinit>
...
of library is relative to the installed kshdb script: C<../lib/kshdb>.
...
The C<kshdb> script option assume a version of ksh with debugging
support, ksh 93t+ 20081210 or later.
...
L<http://bashdb.sourceforge.net> - the homepage for the project for the family of debugger, kshdb, bashdb.
